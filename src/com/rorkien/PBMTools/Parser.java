...
import com.rorkien.PBMTools.formats.PortableBitMap;
import com.rorkien.PBMTools.formats.PortableGrayscaleMap;
import com.rorkien.PBMTools.formats.PortableMapFile;
import com.rorkien.PBMTools.formats.PortablePixelMap;
...
public class Parser {
	 ...
	 * Parses a PBM file.
	 * 
	 * @param filename Path to the PBM file to be parsed
	 * @return The parsed PortableMapFile object
	 */
	public static PortableMapFile parse(String filename) {
		...
		try {
			...
			readcharacter:
			while ((currentCharacter = r.read()) != -1) {
				
				//Check if the actual character is a commentary delimiter. If it is, seeks for the linefeed character. 
				if ((char)currentCharacter == Utils.COMMENTARY_CHARACTER || isCommentary) {
					if (currentCharacter == Utils.LINEFEED_CHARACTER) isCommentary = false;
					else isCommentary = true;
					continue readcharacter;
				}
				
				//While the actual character isn't a whitespace character, inserts it into a buffer
				while (!Utils.isWhitespace(currentCharacter)) {
				...
				switch (segmentsRead) {
					...
					case 1:
						if (buffer.equals("P1")) output = new PortableBitMap();
					...
					case 3:
						...
						//By now we should have the image dimensions, so the matrix size can be calculated
						...
						if (output instanceof PortablePixelMap) tempData = new int[output.getWidth() * output.getHeight() * 3];
						else tempData = new int[output.getWidth() * output.getHeight()];				
						break;
					...
					default:
						tempData[dataPosition] = (int)((1D * Integer.valueOf(buffer) / output.getMaxval()) * 0xFF);						
						dataPosition++;
						break;
				}
