...
public class PortableBitMap extends PortableMapFile {
	...
	/**
	 * Since Java interprets 0 as black and 1 as white, as opposed of the PBM format,
	 * the bytes from the pixel matrix must be flipped before building the image.
	 * 
	 * @return The built BufferedImage
	 */
	@Override
	public BufferedImage buildImage() {	
		//Backs up the original pixel matrix
		int[] origPixels = this.data.clone();
		
		//Flips the image bytes
		for (int i = 0; i < this.data.length; i++) this.data[i] = (this.data[i] + 1) % 2;
